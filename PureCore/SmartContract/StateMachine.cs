...
namespace Pure.SmartContract
{
    public class StateMachine : StateReader
    {
        ...
        public StateMachine(DataCache<UInt160, AccountState> accounts, DataCache<ECPoint, ValidatorState> validators, DataCache<UInt256, AssetState> assets, DataCache<UInt160, ContractState> contracts, DataCache<StorageKey, StorageItem> storages)
        {
            ...
            Register("Pure.Account.SetVotes", Account_SetVotes);
            Register("Pure.Validator.Register", Validator_Register);
            Register("Pure.Asset.Create", Asset_Create);
            Register("Pure.Asset.Renew", Asset_Renew);
            Register("Pure.Contract.Create", Contract_Create);
            Register("Pure.Contract.Migrate", Contract_Migrate);
            Register("Pure.Contract.GetStorageContext", Contract_GetStorageContext);
            Register("Pure.Contract.Destroy", Contract_Destroy);
            Register("Pure.Storage.Put", Storage_Put);
            Register("Pure.Storage.Delete", Storage_Delete);
            #endregion
        }
